...
def create_team(request, user_id):
    try:
        # Get or create Team Leader role
        team_leader_role, _ = Role.objects.get_or_create(
            RoleName="Team Leader",
            defaults={'description': 'Team leader role'}
        )
        
        employee = User.objects.get(id=user_id)
        employee.role = team_leader_role
        employee.save()
        
        # ...rest of your team creation code...
        
    except Exception as e:
        messages.error(request, f"Team creation failed: {str(e)}")
        return redirect('home')
